...
from local_asr_service.backends.base import ASRBackend, TranscriptionResult, TranscriptionStreamEvent
from local_asr_service.config import resolve_cuda_device_index
...
class FasterWhisperBackend(ASRBackend):
    ...
    def _load_model(self):
        if self._model is None:
            ...
            device_index = resolve_cuda_device_index(self.profile.device_index)
    ...
    def transcribe_bytes_stream(
        ...
        *,
        ...
        try:
            ...
            assert tmp_path is not None
            yield TranscriptionStreamEvent(type="progress", message="Audio loaded, ASR decode started")
            segments_iter, info = model.transcribe(
                str(tmp_path),
                language=lang_arg,
                vad_filter=self.profile.vad_filter,
            ...
            for seq, seg in enumerate(segments_iter, start=1):
                text = seg.text.strip()
                if not text:
                    continue
                segment = TranscriptSegment(
